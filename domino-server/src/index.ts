...
app.get("/", (_req, res) => {
  res.json({ status: "ok", message: "Domino game server running" });
});
...
class LearningAI {
  ...
  strategyWeights: Record<WeightKeys, number>;
  ...
  constructor(
    ...
    weightsOverride?: Record<WeightKeys, number>,
  ) {
  ...
  initializeWeights(): Record<WeightKeys, number> {
    const baseWeights = {
  ...
  evaluateMove(
    ...
    const handSafety = numbersInHand.size * 5 - handPoints * 2;
  ...
  evaluatePrediction(
    ...
    for (const move of playable) {
      for (const moveSide of move.sides) {
        let s = 0;
        if (move.domino.left === move.domino.right) s += 10;
        s += move.domino.left + move.domino.right;
  ...
  learnFromGame(game: GameState, result: "win" | "loss" | "draw") {
    ...
    for (let i = this.experienceBuffer.length - 1; i >= 0; i -= 1) {
      ...
      const sm = this.getOrCreateState(exp.state);
      const oldQ = sm.get(exp.action) ?? 0;
      const updated = oldQ + this.learningRate * (discounted - oldQ);
      sm.set(exp.action, updated);
...
function getPlayableDominoes(
  ...
  for (const domino of hand) {
    ...
    if (canPlay) playable.push({ domino, sides });
  }
  return playable;
}
...
function aiSelectMove(
  ...
  if (learningAI) return learningAI.chooseMove(game, player);
  ...
  for (const { domino, sides } of playable) {
    for (const side of sides) {
      let score = 0;
      if (domino.left === domino.right) score += 20;
...
function processAITurn(game: GameState): void {
  const currentPlayer = game.players[game.currentPlayerIndex];
...
// ─── Re-sort helper ───────────────────────────────────────────────────────────
function resortPlayers(game: GameState) {
...
io.on("connection", (socket) => {
  ...
  socket.on("addAIPlayer", ({ gameId, team, difficulty }) => {
    ...
    if (!team) {
      const team1Count = game.players.filter((p) => p.team === "team1").length;
      const team2Count = game.players.filter((p) => p.team === "team2").length;
      targetTeam = team1Count <= team2Count ? "team1" : "team2";
    }

    const teamCount = game.players.filter((p) => p.team === targetTeam).length;
    if (teamCount >= 2) {
...
httpServer.listen(port, () => {
  console.log(`> Domino server running on http://0.0.0.0:${port}`);
  console.log(
    `> CORS: ${Array.isArray(allowedOrigins) ? allowedOrigins.join(", ") : allowedOrigins}`,
  );
});
